...
fn main() {
    ...
    // The weights:
    println!("Les poids :");
    let matrix_weight_l0: Vec<f64> = runst::weight_init::normal_dis(LAYER[0], LAYER[1]);
    println!("La matrix des poids entre la couche 0(input) et 1 :");
    println!("{:?}\n", matrix_weight_l0);
   
    let matrix_weight_l1: Vec<f64> = runst::weight_init::normal_dis(LAYER[1], LAYER[2]);
    println!("La matrix des poids entre la couche 1 et 2 :");
    println!("{:?}\n", matrix_weight_l1);

    // Creation of the tensor:
    let tensor: [&Vec<f64>; 2] = [&matrix_weight_l0, &matrix_weight_l1];

    // The bias:
    let bias_l1: Vec<f64> = vec![0.0; LAYER[1]];
    println!("Les biais de la couche 1 sont :");
    println!("{:?}\n", bias_l1);

    let bias_l2: Vec<f64> = vec![0.0; LAYER[2]];
    println!("Les biais de la couche 2 sont :");
    println!("{:?}\n", bias_l2);

    // Creation of the bias of matrix:
    let bias_matrix: [&Vec<f64>; 2] = [&bias_l1, &bias_l2];
    ...
    */
    ...
    let mut b3_l1_find: Vec<bool> = [false; bias_matrix[1].len()];
    ...
    */
    ...
    // pour le nombre d'essayes indiqué
        ...
        if b3_l1_find == false {
            ...
            // determination de la prochaine valeur du coéficient directeur
            bias_matrix[1] = bias_matrix[1] - step_size;
